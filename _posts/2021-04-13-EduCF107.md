...
---
### 결과  
내가 실제로 참여한 8번째 대회이다. Div2 대회에서는 4솔을 한 적도 없고 그 정도 실력이 안된다고 생각했었는데, 이번 Round에서 4솔을 했다..!
...
$array$의 크기는 $n = 3*10^5$, $query$의 개수는 $q = 3*10^5$ 이므로 naive하게 구현하면 $O(nq)$가 되어 시간 초과가 발생한다.  
...
만약 $have[m][n]$가 $p \geq 2$이면 $cost$에 ${}_{p}\mathrm{C}_{2}$가 더해진다.  
