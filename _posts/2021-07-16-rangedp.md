---
layout: post
title: "구간 DP를 푸는 요령 (백준 11049, 11066)"
date: 2021-07-16
categories:
- Midium_Algorithm
tags:
...
### 구간 DP 소개
Dymanic Programming에 항상 따라오는 **점화식**을 적극 활용하는 방법이 구간 DP이다.  

가장 간단한 DP문제인 피보나치 수열의 점화식은  
$ F(0) = 1, F(1) = 1 $  
$ F(i) = F(i-1) + F(i-2), (i \geq 2)$  
이다.  
이런 문제는 점화식의 변수가 한 개이나, 구간 DP는 변수가 2, 3개 이상인 점화식을 도출에 푸는 방식이다.

구간 DP 중 유명한 예로 **플로이드-와샬** 알고리즘이 있다.  
...
이 2문제는 위의 플로이드-와샬과 비슷한 점화식 도출 난이도, 구현 난이도를 가지는 문제들이다.  
...
$dp(i,j)$에서 $j-i$가 작은 것부터 구해나가는 것이 특징이다.  

**점화식**  
$ dp(i,j) $ : $i$번째~$j$번째까지 곱하는데 필요한 곱셉 횟수  
...
**점화식**  
$ dp(i,j) $ : $i$번째~$j$번째까지 합치는데 필요한 비용  
$ dp(i,i) = 0 ( 0 \leq i \leq n-1) $  
$ dp(i,j) = min_{k=i}^{j-1}(dp(i,k)+dp(k,j)+\sum_{x=i}^j arr[i]) (j-i \geq 1)$  
$\sum_{x=i}^j arr[i]$는 누적합을 이용해 구해주면 된다. 
