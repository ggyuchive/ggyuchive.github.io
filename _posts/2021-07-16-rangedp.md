...
**점화식**  
$ dp(i,j) $ : $i$~$j$번째 문자가 팰린드롬이면 1, 아니면 0   
$ dp(i,i) = 1 ( 0 \leq i \leq n-1) $ 


$ dp(i,i+1)= \begin{cases}1(arr[i]=arr[i+1])\\0 (else)\end{cases} (0 \leq i \leq n-2) $  
$ dp(i,j) =\begin{cases} 1 (arr[i]=arr[j], dp(i+1,j-1)=1)\\0 (else)\end{cases}  (j-i \geq 2)$
