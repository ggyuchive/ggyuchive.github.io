...
이다.  
이런 문제는 점화식의 변수가 한 개이나, 구간 DP는 변수가 2, 3개 이상인 점화식을 도출해 푸는 방식이다.
...
아래 2문제는 위의 플로이드-와샬과 비슷한 점화식 도출 난이도(점화식 형태도 비슷하다..), 구현 난이도를 가지는 문제들이다.  
두 문제 다 $dp(i,j)$에서 $j-i$가 작은 것부터 구해나가는 것이 특징이다.  
